...
static void iolink_dl_message_h_sm_await_reply16 (iolink_port_t * port)
{
   ...
   else if (dl->dataready)
   {
      ...
      dl->txerror)
   {
      if (dl->message_handler.retry >= 3)
      {
         LOG_ERROR (
            IOLINK_DL_LOG,
            "%s: %s: %s on Port %d (retries exhausted)\n",
            __func__,
            iolink_dl_mh_st_literals[dl->message_handler.state],
      ...
      else
      {
         LOG_WARNING (
            IOLINK_DL_LOG,
            "%s: %s: %s on Port %d - retry %d/3\n",
            __func__,
            iolink_dl_mh_st_literals[dl->message_handler.state],
            (dl->rxtimeout) ? "RXTimeout" : "RXError",
            iolink_get_portnumber (port),
            dl->message_handler.retry + 1);
         dl->rxtimeout = false;
         dl->rxerror = false;
         dl->txerror = false;
         dl->message_handler.retry++;
   ...
   while (true)
   {
      if (!os_event_wait (dl->event, mask, &dl->triggered_events, event_timeout))
      {
         ...
         if (dl->triggered_events & IOLINK_PL_EVENT_RXRDY)
         {
            ...
            {
               // FIFO read failed - drive state machine with error
               // to prevent permanent hang in AW_REPLY state
               LOG_WARNING (IOLINK_DL_LOG,
                  "RXRDY but get_data failed - signaling rx error\n");
               dl->rxerror = true;
               iolink_dl_message_h_sm (port);
               dl->rxerror = false;
            }
